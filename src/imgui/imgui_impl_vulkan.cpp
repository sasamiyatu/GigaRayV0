...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass, VkFormat colorFormat, VkFormat depthFormat)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ...
    rendering_create_info.pColorAttachmentFormats = &colorFormat;
    rendering_create_info.depthAttachmentFormat = depthFormat;
...
bool ImGui_ImplVulkan_CreateDeviceObjects(VkFormat colorFormat, VkFormat depthFormat)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkResult err;
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, bd->RenderPass, v->MSAASamples, &bd->Pipeline, bd->Subpass, colorFormat, depthFormat);
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass, VkFormat color_format, VkFormat depth_format)
{
    ...
    ImGui_ImplVulkan_CreateDeviceObjects(color_format, depth_format);
